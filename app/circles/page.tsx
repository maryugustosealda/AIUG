      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {circles.map((c) => (
          <Link key={c.id} href={`/circles/${c.slug}`} className="card overflow-hidden transition hover:border-brand-300 dark:hover:border-brand-700">
            <div className="h-20 bg-gradient-to-br from-brand-400/40 to-brand-700/40" />
            <div className="p-4">
              <div className="flex items-center gap-2">
                <div className="-mt-10 grid h-12 w-12 place-items-center rounded-xl bg-[rgb(var(--card))] text-2xl">{c.icon || c.name.slice(0, 1)}</div>
