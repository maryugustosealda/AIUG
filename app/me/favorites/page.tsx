...
export default async function MeFavoritesPage() {
  const session = await auth();
  if (!session?.user) redirect("/login?callbackUrl=/me/favorites");
  ...
  const items = favs.map((f) => f.post).filter((p) => p && p.status === "published");
  ...
  return (
    <div className="mx-auto max-w-lg space-y-4">
      ...
      <ul className="card divide-y divide-[rgb(var(--border))]">
        {items.length === 0 && (
          <li className="p-8 text-center text-sm text-[rgb(var(--muted))]">还没有收藏</li>
        )}
        {items.map((p) => (
          <li key={p!.id}>
